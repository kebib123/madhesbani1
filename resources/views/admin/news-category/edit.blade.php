...
<form class="form-horizontal" role="form" action="{{ route('newscategory.update', $data->id) }}" method="post" enctype="multipart/form-data">
 {{ csrf_field() }}            
 ...
 <div class="col-md-9">
  ...
  <div class="panel">
      ...
      <div class="col-lg-8">                  
       <div class="bs-component">
        <select class="form-control" name="news_type">
          ...
          <option value="{{$row->id}}" {{ ($row->id == $data->news_type)?"selected":"" }}>{{$row->news_type}}</option>
          @endforeach
          @endif
        </select>
    ...
    <div class="form-group">
      ...
      <div class="col-lg-8">
        <div class="bs-component">
          <input type="text" id="inputStandard" name="uri" class="form-control" value="{{$data->uri}}"  placeholder="" />
        </div>
      </div>
    </div>     
